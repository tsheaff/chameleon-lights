...
class Cascade(Animator):
    ...
    def __init__(self):
        ...
        self.starting_position = random.uniform(
            Cascade.MIN_STARTING_POSITION, Cascade.MAX_STARTING_POSITION
        )
    ...
    def update_with_progress(self, curved_progress):
        ...
        for i in range(start_index, min(end_index + 1, self.num_pixels)):
            ...
            actual_color = (
                full_color
                if color_ratio is 1
                else helpers.interpolate_colors(
                    self.previous_colors[i], full_color, color_ratio
                )
            )
...
class Twinkle(Animator):
    ...
    def __init__(self):
        ...
        self.twinkle_periods = list(
            map(
                lambda n: random.uniform(
                    Twinkle.MIN_TWINKLE_PERIOD, Twinkle.MAX_TWINKLE_PERIOD
                ),
                [0] * self.num_pixels,
            )
        )
    ...
    def update_frame(self):
        ...
        curved_buffer_progress = (
            0 if buffer_progress is 0 else NormalizedEaseInOut.ease(buffer_progress)
        )
        if progress >= 1:
        ...
        for i, period in enumerate(self.twinkle_periods):
            ...
            conductor.pixel_colors[i] = helpers.interpolate_colors(
                black, original_color, buffered_intensity
            )
...
class Sweep(Animator):
    ...
    def update_frame(self):
        ...
        for i in range(0, self.num_pixels):
            ...
            original_color = self.previous_colors[i]
            conductor.pixel_colors[i] = helpers.interpolate_colors(
                inverted_color, original_color, buffered_intensity
            )
...
class Conductor:
    ...
    def __init__(self):
        self.pixels = neopixel.NeoPixel(
            Conductor.PIN, Conductor.NUM_PIXELS, auto_write=False
        )
        self.pixel_colors = list(
            map(lambda x: Color("#000000"), [None] * Conductor.NUM_PIXELS)
        )
